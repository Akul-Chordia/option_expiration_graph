...
print("Enter : quantity|option_type|strike|price|quantity")
    ...
    for position in positions:
        if position.type == "call":
            if strike <= position.strike:
                cost -= position.qty * position.price
            else:
                cost += position.qty * (strike - position.strike - position.price)
        else:
            if strike >= position.strike:
                cost -= position.qty * position.price
            else:
                cost += position.qty * (position.strike - strike - position.price)
